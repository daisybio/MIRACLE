			...
			<g:each in="${1..(plateLayout.rows)}" var="row">
                <tr id="r${row}">
					...
					<g:each in="${1..(plateLayout.cols)}">
                            <td
							style="border: 1px solid; background-color:${wellList.get(well)?.properties[sampleProperty]?wellList.get(well).properties[sampleProperty].color?:'#e0e0e0':''};"><input
							name="${wellList.get(well).id}" type="hidden" value=""></td>
